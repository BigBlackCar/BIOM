...
int verificar_grupo(alimento_tipo *al, int *qtd)
{
    int flag = 0;
...
int validar_nome(pessoa_tipo *pt, char nome[], int *qtd2)
{
    int flag = 0, i = 0, b;
...
int validar_nome_procura(pessoa_tipo *pt, int *qtd2, char nomeproc[])
{
    int flag = 0, a, i = 0;
...
void mostrar_dietas(pessoa_tipo *pt, dieta_tipo *dt, alimento_tipo *at, int *qtd3, int *qtd, int *qtd2)
{
    ...
    if ( *qtd3 > 0)
    {
        printf("Nome da pessoas a procurar: ");
        scanf(" %[^\n]", &nomeproc);
        do
        {
            ...
            if (flag == 0)
                printf("Nome inválido! Introduza um nome existente");
...
void estimar_valor_kcal(alimento_tipo *al, int *qtd)  //calcula as calorias de uma refeição atraves de alimentos ingeridos e as respetivas quantidades (TD)
{
    ...
    do
    {
        do
        {
            printf("Nome do alimento que ingeriu (s/S para parar): "); //inserir -1 acaba o ciclo e calcula a kcal dos alimentos introduzidos
            scanf(" %[^\n]",&nomealim);
            round = round + 1;
            do
            {
...
void eliminar_pessoas(pessoa_tipo *pt, int *qtd2)// eliminar pessoas
{
    ...
    do
    {
        if( strcmp( pt[t].nome, eli ) == 0 )//compara o nome escrito pelo utilizador com todos os nomes no programa
        {
            system ("cls");
            strcpy(pt[t].nome,z);
            sort_structs_pessoas(pt,*qtd2); //passa a pessoa com o nome inserido para ultimo pois agora chama-se ~~~~~~
...
{
    int i = 0, j = 0, a, l = 0;//variavel numerica
    char gen;//variavel pedida ao utilizador para comparar com o sexo das pessoas no sistema
    char pool[100];
...
void balanca_alimentar(dieta_tipo *dt, int *qtd3)
{
...
/*void listar_alimentos_mais_consumidos(dieta_tipo *dt, pessoa_tipo *pt, int *qtd, int *qtd3)
{
    int a[*qtd], i = 0, j = 0, k = 0, l = 0;

    while(i < *qtd)
    {
        while(j < 5)
        {
            while(k < dt[j].quantalim[l])
            {
...
void grafico_barras(dieta_tipo *dt, int *qtd3)
{
...
int	submenu_4(int *qtd2, int *qtd, pessoa_tipo *pt, alimento_tipo *at, dieta_tipo *dt, int *qtd3)
{
    char    op;
    do
    {
        ...
        case '2':
            system("cls");
            //listar_alimentos_mais_consumidos(dt,pt,qtd,qtd3);
            break;
